...
const WeatherData = ({ capital }) => {
  ...
  if (!isLoading) {
    // aqui tienes que comprobar que weather_icons no sea undefined

    const icon =
      (weather && weather.current && weather.current.weather_icons) || '';
    const description =
      (weather &&
        weather.current &&
        weather &&
      ...
      '';
    const windSpeed =
      (weather && weather.current && weather && weather.current.wind_speed) ||
      '';
    const windDirection =
      (weather && weather.current && weather && weather.current.wind_dir) || '';
